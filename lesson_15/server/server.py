...
class WorkingServer(threading.Thread, metaclass=ServerVerifier):
    ...
    def read_reaquest(self):
        '''Метод получения поступающих собщений.'''
    ...
    def create_answer(self):
        '''Метод обработки сообщений и подготовки ответов пользователям.'''
        global change_count_connection
        while self.request_dict:
            ...
            # обработка сообщения на добовление контакта
                ...
                answer = f'Пользователь {message["user_login"]} добавлен в список контактов' \
                         f' пользователя {message["user_id"]}' \
                    if self.database.add_contact(message["user_id"], message["user_login"]) \
                    else f'Пользователь {message["user_login"]} уже есть в списке контактов ' \
                         f'пользователя {message["user_id"]}'
            ...
            # обработка сообщения на удаление контакта
                    ...
                    'user_id' in message and 'user_login' in message:
                answer = f'Пользователь {message["user_login"]} удален из списка контактов ' \
                         f'пользователя {message["user_id"]}' \
                    if self.database.del_contact(message["user_id"], message["user_login"]) \
                    else f'Пользователя {message["user_login"]} нет в списке контактов ' \
                         f'пользователя {message["user_id"]}'
                self.answer_dict[client] = create_server_message(201, answer)
                server_log.info(answer)
    ...
    def send_answer(self):
        '''Метод рассылки ответаов пользователям.'''
        global change_count_connection
...
def main():
    ...
    def list_users_update():
        '''Функция заполняющий таблицу активных пользователей.'''
        global change_count_connection
    ...
    def show_user_history():
        '''Функция создающий окно со статистикой клиентов.'''
    ...
    def show_config_server():
        '''Функия создающий окно с настройками сервера.'''
        global config_window
        config_window = ConfigWindow()

    def show_register_user():
        '''Функция создающий окно регистрации пользователя.'''
        global register_window
        register_window = RegisterWindow(server.database, server.socket)

    def show_remove_user():
        '''Функция создающий окно удаления пользователя.'''
